...
export async function GET(req: Request) {
  ...
  const items = await prisma.outreachCampaign.findMany({
  ...
  return NextResponse.json({
    ...
    items: items.map(({ recipients, ...item }) => ({
      ...item,
      statusCounts: recipients.reduce((all: Record<string, number>, row) => {
...
export async function POST(req: Request) {
  ...
  const bodyText = String(
    body?.bodyText || outreachHtmlToText(bodyHtml),
  ).trim();
    ...
    return NextResponse.json(
      {
        ok: false,
        error:
          "Campaign name, subject, message, and at least one contact are required.",
      },
      { status: 400 },
    );
  if (contactIds.length > MAX_CAMPAIGN_CONTACTS)
    return NextResponse.json(
      {
        ok: false,
        error: `Select no more than ${MAX_CAMPAIGN_CONTACTS} contacts for each campaign.`,
      },
      { status: 400 },
    );
  ...
  const suppressions = await prisma.outreachSuppression.findMany({
    where: {
      normalizedEmail: {
        in: contacts.map((x) => x.normalizedEmail).filter(Boolean) as string[],
      },
    },
  ...
  const eligible = contacts.filter((contact) => {
    if (
      ...
      contact.consentBasis === "UNASSESSED" ||
    ...
    return NextResponse.json(
      {
        ok: false,
        error:
          "No selected contacts are currently sendable. Record a valid contact basis and source, and ensure the address is not suppressed.",
      },
      { status: 400 },
    );
  ...
  await auditLog({
    action: "ADMIN_OUTREACH_CAMPAIGN_CREATE",
    userId: gate.user.id,
