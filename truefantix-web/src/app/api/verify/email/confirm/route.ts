...
export async function POST(req: Request) {
  try {
    ...
    const validation = await validateRequest(schemas.verificationCodeConfirm)(
      req,
    );
    ...
    return await runOrdinaryVerificationOperation(userId, async (tx) => {
      ...
      const vc = await tx.verificationCode.findFirst({
        where: {
          userId: user.id,
          kind: "EMAIL",
      ...
      if (!vc) {
        return jsonError(
          400,
          "NO_ACTIVE_CODE",
          "No active code found. Please request a new code.",
        );
      }
      ...
      // Mark the code used and verify the current destination atomically.
      await tx.verificationCode.update({
        where: { id: vc.id },
      ...
      return NextResponse.json(
        { ok: true, verified: "EMAIL" },
        { status: 200 },
      );
    });
  } catch (error) {
    if (error instanceof ManagedAccountVerificationOperationError) {
