...
import {
  ManagedAccountVerificationOperationError,
  runOrdinaryVerificationOperation,
} from "@/lib/verification/ordinary-user";
...
export async function POST(req: Request) {
  try {
    ...
    return await runOrdinaryVerificationOperation(userId, async (tx) => {
      ...
      if (user.isBanned)
        return jsonError(403, "BANNED", "This account is restricted.");
      if (!user.phone) {
        return jsonError(
          400,
          "PHONE_MISSING",
      ...
      const sendsLastHour = await tx.verificationCode.count({
        where: {
          userId: user.id,
          kind: "PHONE",
      ...
      const active = await tx.verificationCode.findFirst({
        where: {
          userId: user.id,
          kind: "PHONE",
      ...
      if (active?.lastSentAt) {
        const secondsSince = Math.floor(
          (now.getTime() - active.lastSentAt.getTime()) / 1000,
        );
      ...
      if (!smsResult.ok) {
        return jsonError(
          503,
          "SMS_DELIVERY_FAILED",
          smsResult.error ?? "Could not send verification text.",
        );
      }
    ...
    if (err instanceof ManagedAccountVerificationOperationError) {
      return stagingConsoleOnlyError();
    }
    console.error("POST /api/verify/phone/send error:", err);
    const message =
      err instanceof Error ? err.message : "Could not send phone code.";

    return jsonError(500, "PHONE_SEND_FAILED", message);
  }
}
