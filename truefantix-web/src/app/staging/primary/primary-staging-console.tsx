...
type BuyerJourney = { generation: number; event: { id: string; title: string; status: string }; ticketType: { name: string; basePriceMinor: number; currency: string }; reservation: { id: string; status: string; quantity: number } | null; order: { id: string; status: string; grossTotalMinor: number; currency: string } | null; payment: { status: string; providerIntentId: string | null } | null; admission: { id: string; status: string; credentialId: string | null; scans: Array<{ result: string; scannedAt: string }> } | null };
type ConsoleState = { actor: Actor; organizers: Organizer[]; audit: AuditEvent[]; refundConsole: RefundConsoleState | null; buyerJourney: BuyerJourney | null };
    ...
    <main className="mx-auto min-h-screen max-w-6xl space-y-6 bg-slate-50 px-4 py-8 text-slate-950 sm:px-6">
      ...
      {actor && state && (
        <section className="rounded-2xl border border-emerald-200 bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-emerald-700">Synthetic buyer purchase and admission</p><h2 className="mt-1 text-xl font-semibold">Buyer journey checkpoint</h2><p className="mt-1 text-sm text-slate-600">Advance one durable local step at a time. No Stripe request, email, webhook, payout, or live credential is used.</p></div>{actor.role === "ADMIN" && <button className={secondaryButtonClass} disabled={busy} onClick={() => void runAction({ action: "reseedBuyerJourney" }, "Fresh synthetic buyer journey seeded.")}>{state.buyerJourney ? "Start fresh generation" : "Seed buyer journey"}</button>}</div>
          {!state.buyerJourney ? <p className="mt-4 rounded-lg bg-emerald-50 p-3 text-sm">An admin reviewer must seed the journey.</p> : <div className="mt-4 space-y-3 text-sm"><p><strong>{state.buyerJourney.event.title}</strong> · {state.buyerJourney.ticketType.name} · {(state.buyerJourney.ticketType.basePriceMinor / 100).toLocaleString("en-CA", { style: "currency", currency: state.buyerJourney.ticketType.currency })}</p><div className="grid gap-2 sm:grid-cols-5">{[["Reservation", state.buyerJourney.reservation?.status ?? "READY"], ["Order", state.buyerJourney.order?.status ?? "NOT_CREATED"], ["Payment", state.buyerJourney.payment?.status ?? "NOT_STARTED"], ["Ticket", state.buyerJourney.admission ? "ISSUED" : "NOT_ISSUED"], ["Admission", state.buyerJourney.admission?.status ?? "NOT_ADMITTED"]].map(([label, value]) => <div className="rounded-lg bg-emerald-50 p-3" key={label}><span className="text-xs text-slate-600">{label}</span><br /><strong>{value}</strong></div>)}</div>{state.buyerJourney.admission?.credentialId && <p className="text-xs text-slate-500">Opaque synthetic credential: {state.buyerJourney.admission.credentialId} · accepted scans: {state.buyerJourney.admission.scans.filter((scan) => scan.result === "ACCEPTED").length}</p>}{actor.role === "ADMIN" && state.buyerJourney.admission?.status !== "CHECKED_IN" && <button className={buttonClass} disabled={busy} onClick={() => void runAction({ action: "advanceBuyerJourney" }, "Synthetic buyer journey advanced one step.")}>Advance next step</button>}</div>}
        </section>
      )}
                ...
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  {tickets.map((ticket) => {
                    ...
                    return <div key={ticket.id} className="rounded-lg bg-slate-50 p-3 text-xs">
                      ...
                      <span className="text-slate-600">refund {ticketRefund?.status ?? "none"} · revocations {ticket.revocations.length}</span>
                      ...
                      {attempt && <div className="mt-2 text-slate-700"><strong>Synthetic provider evidence:</strong> {attempt.status} · {attempt.providerRefundId ?? "not attached"}</div>}
