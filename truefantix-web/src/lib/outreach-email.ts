...
export async function sendOutreachEmail(input: { to:string; subject:string; text:string; unsubscribeUrl:string; replyTo?:string }): Promise<OutreachEmailResult> {
  ...
  if(resendKey){
    const response=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${resendKey}`,"Content-Type":"application/json"},body:JSON.stringify({from:outreachSender(),to:[input.to],reply_to:input.replyTo||from,subject:input.subject,text:input.text,headers})});
