...
async function requireSyntheticNoDeliveryIntent(tx: Tx) {
  const [purchaseAllocations, refunds, refundItems, refundAllocations, refundAttempts, refundProviderEvents, revocations, cancellations, cancellationBatches, cancellationBatchTickets, cancellationSnapshots, cancellationRefundLinks, obligations, checkedInApprovals, waiverApprovals, auditEvents] = await Promise.all([
    ...
    tx.primaryRefundAllocation.findMany({
      where: { refund: { organizerId: ORGANIZER_ID } },
      select: { id: true },
    }),
    tx.primaryRefundProviderAttempt.findMany({
      where: { refund: { organizerId: ORGANIZER_ID } },
      select: { id: true },
    }),
    ...
    tx.primaryCancellationBatchTicket.findMany({
      where: { cancellation: { organizerId: ORGANIZER_ID } },
      select: { id: true },
    }),
    tx.primaryCancellationSnapshotTicket.findMany({
      where: { cancellation: { organizerId: ORGANIZER_ID } },
      select: { id: true },
    }),
    ...
    tx.primaryCheckedInRefundApproval.findMany({
      where: { refund: { organizerId: ORGANIZER_ID } },
      select: { id: true },
    }),
  ...
  const opaqueWorkflowAggregateIds = [
    ...
    ...cancellationBatchTickets.map((ticket) => ticket.id),
