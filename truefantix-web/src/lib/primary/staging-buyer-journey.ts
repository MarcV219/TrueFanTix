...
function requireOrderState(order: Prisma.PrimaryOrderGetPayload<{ include: { lines: true; components: true } }>, scope: ReturnType<typeof ids>, buyerId: string, hasPayment: boolean, reservationCreatedAt: Date, now: Date) {
  ...
  if (
    ...
    || order.createdAt.getTime() < reservationCreatedAt.getTime() || order.createdAt.getTime() > now.getTime()
...
function requireAdmissionState(ticket: Prisma.PrimaryAdmissionTicketGetPayload<{ include: { credential: true; scans: true } }>, scope: ReturnType<typeof ids>, buyerId: string, actorId: string, paidAt: Date, now: Date) {
  ...
  const checkedIn = ticket.status === "CHECKED_IN" && ticket.scans.length === 1 && scan
    ...
    && scan.scannedAt.getTime() <= now.getTime();
  if (
    ...
    || ticket.unitNumber !== 1 || ticket.issuanceIdempotencyKey !== `${scope.base}:issue`
    || ticket.issuedAt.getTime() < paidAt.getTime() || ticket.issuedAt.getTime() > now.getTime()
...
export async function advancePrimaryStagingBuyerJourney(db: PrismaClient, actor: Actor) {
  return db.$transaction(async (tx) => {
    ...
    requireOrderState(order, scope, buyer.id, Boolean(payment), reservation.createdAt, now);
    if (!payment) {
    ...
    requireAdmissionState(ticket, scope, buyer.id, actor.id, order.paidAt!, now);
