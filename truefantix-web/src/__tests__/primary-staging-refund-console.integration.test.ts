...
if (!databaseUrl) describe.skip("primary staging refund console PostgreSQL integration", () => {
  ...
  it("rejects delivery intent scoped only by the opaque synthetic owner membership", async () => {
    const seeded = await reseedPrimaryStagingRefundScenario(db, admin);
    const ordinaryBase = `staging-refund-g${seeded.generation}-ordinary`;
    const membership = await db.primaryOrganizerMembership.findUniqueOrThrow({
      where: {
        organizerId_userId: {
          organizerId: "primary-staging-refund-organizer",
          userId: organizer.id,
    ...
    const outbox = await db.primaryOutboxMessage.create({ data: {
      topic: "synthetic.unexpected.unscoped-membership-delivery",
      aggregateType: "PrimaryOrganizerMembership",
      aggregateId: membership.id,
      payloadJson: { synthetic: true },
      idempotencyKey: `${ordinaryBase}:unexpected-unscoped-membership-delivery-intent`,
    } });
  ...
  it.each([
    ["organizer", "organizer@primary-staging.example.invalid"],
    ["administrator", "admin@primary-staging.example.invalid"],
  ])("rejects delivery intent scoped only by the opaque reserved %s identity", async (persona, email) => {
    const seeded = await reseedPrimaryStagingRefundScenario(db, admin);
    const ordinaryBase = `staging-refund-g${seeded.generation}-ordinary`;
    const user = await db.user.findUniqueOrThrow({ where: { email } });
    const outbox = await db.primaryOutboxMessage.create({ data: {
      topic: `synthetic.unexpected.unscoped-${persona}-delivery`,
      aggregateType: "User",
      aggregateId: user.id,
      payloadJson: { synthetic: true },
      idempotencyKey: `${ordinaryBase}:unexpected-unscoped-${persona}-delivery-intent`,
    } });
  ...
  it("rejects delivery intent scoped only by the reserved refund policy", async () => {
    const seeded = await reseedPrimaryStagingRefundScenario(db, admin);
    const ordinaryBase = `staging-refund-g${seeded.generation}-ordinary`;
    const outbox = await db.primaryOutboxMessage.create({ data: {
      topic: "synthetic.unexpected.unscoped-policy-delivery",
      aggregateType: "PrimaryRefundPolicyVersion",
      aggregateId: "primary-refund-policy-v1",
      payloadJson: { synthetic: true },
      idempotencyKey: `${ordinaryBase}:unexpected-unscoped-policy-delivery-intent`,
    } });
  ...
  it("rejects delivery intent scoped only by an opaque synthetic obligation", async () => {
    const seeded = await reseedPrimaryStagingRefundScenario(db, admin);
    const cancellationBase = `staging-refund-g${seeded.generation}-cancellation`;
    await runPrimaryStagingRefundAction(db, organizer, "activateCancellation", {});
    await runPrimaryStagingRefundAction(db, organizer, "prepareCancellation", {});
    const waiverObligation = await db.primaryRefundObligation.findUniqueOrThrow({
      where: { idempotencyKey: `${cancellationBase}:obligation:waiver` },
    });
    const outbox = await db.primaryOutboxMessage.create({ data: {
      topic: "synthetic.unexpected.unscoped-obligation-delivery",
      aggregateType: "PrimaryRefundObligation",
      aggregateId: waiverObligation.id,
      payloadJson: { synthetic: true },
      idempotencyKey: `${cancellationBase}:unexpected-unscoped-obligation-delivery-intent`,
    } });

    await expect(runPrimaryStagingRefundAction(db, admin, "approveCancellationWaiver", {
      reason: "Must not approve after foreign obligation delivery intent",
      evidence: "synthetic-unscoped-obligation-delivery",
    })).rejects.toMatchObject({
      code: "STAGING_REFUND_DELIVERY_INTENT_INVALID",
      generation: seeded.generation,
    });
    ...
    await expect(db.primaryObligationWaiverApproval.count({
      where: { obligationId: waiverObligation.id },
    })).resolves.toBe(0);
  ...
  it("rejects delivery intent scoped only by an opaque synthetic purchase allocation", async () => {
    const seeded = await reseedPrimaryStagingRefundScenario(db, admin);
    const ordinaryBase = `staging-refund-g${seeded.generation}-ordinary`;
    const allocation = await db.primaryPurchaseAllocation.findFirstOrThrow({
      where: { orderId: `${ordinaryBase}-order` },
      orderBy: { id: "asc" },
    });
    const outbox = await db.primaryOutboxMessage.create({ data: {
      topic: "synthetic.unexpected.unscoped-allocation-delivery",
      aggregateType: "PrimaryPurchaseAllocation",
      aggregateId: allocation.id,
      payloadJson: { synthetic: true },
      idempotencyKey: `${ordinaryBase}:unexpected-unscoped-allocation-delivery-intent`,
    } });
  ...
  it("rejects delivery intent scoped only by an opaque synthetic refund provider event", async () => {
    const seeded = await reseedPrimaryStagingRefundScenario(db, admin);
    const checkedBase = `staging-refund-g${seeded.generation}-checked`;
    await runPrimaryStagingRefundAction(db, organizer, "requestCheckedRefund", {});
    await runPrimaryStagingRefundAction(db, admin, "approveCheckedRefund", {
    ...
    await runPrimaryStagingRefundAction(db, organizer, "completeCheckedRefund", {});
    const providerEvent = await db.primaryRefundProviderEvent.findFirstOrThrow({
      where: { attempt: { refund: { requestKey: `${checkedBase}:refund:checked` } } },
    });
    const outbox = await db.primaryOutboxMessage.create({ data: {
      topic: "synthetic.unexpected.unscoped-refund-provider-event-delivery",
      aggregateType: "PrimaryRefundProviderEvent",
      aggregateId: providerEvent.id,
      payloadJson: { synthetic: true },
      idempotencyKey: `${checkedBase}:unexpected-unscoped-refund-provider-event-delivery-intent`,
    } });
  ...
  it("rejects delivery intent scoped only by an opaque synthetic cancellation batch ticket", async () => {
    const seeded = await reseedPrimaryStagingRefundScenario(db, admin);
    const cancellationBase = `staging-refund-g${seeded.generation}-cancellation`;
    await runPrimaryStagingRefundAction(db, organizer, "activateCancellation", {});
    await runPrimaryStagingRefundAction(db, organizer, "prepareCancellation", {});
    ...
    await runPrimaryStagingRefundAction(db, organizer, "completeCancellation", {});
    const batchTicket = await db.primaryCancellationBatchTicket.findFirstOrThrow({
      where: { cancellation: { requestKey: `${cancellationBase}:cancellation` } },
    });
    const outbox = await db.primaryOutboxMessage.create({ data: {
      topic: "synthetic.unexpected.unscoped-cancellation-batch-ticket-delivery",
      aggregateType: "PrimaryCancellationBatchTicket",
      aggregateId: batchTicket.id,
      payloadJson: { synthetic: true },
      idempotencyKey: `${cancellationBase}:unexpected-unscoped-cancellation-batch-ticket-delivery-intent`,
    } });
