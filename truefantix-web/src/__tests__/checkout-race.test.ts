...
jest.mock("@/lib/prisma", () => ({
  prisma: {
    ...
    seller: { findUnique: jest.fn(), updateMany: jest.fn() },
    accessTokenTransaction: { createMany: jest.fn() },
    $queryRaw: jest.fn(),
    $transaction: jest.fn(),
  },
}));
...
const mockedPrisma = prisma as unknown as {
  ...
  seller: { findUnique: jest.Mock; updateMany: jest.Mock };
  accessTokenTransaction: { createMany: jest.Mock };
  $queryRaw: jest.Mock;
...
const walletId = "cm1234567890abcdefghijkl";
const ticketId = "cm2234567890abcdefghijkl";
const ordinaryUser = {
  id: "user-1",
...
function request(buyerSellerId = walletId) {
  return new Request("http://localhost/api/orders/checkout", {
    method: "POST",
    headers: { "content-type": "application/json", "idempotency-key": "checkout-key-1" },
    body: JSON.stringify({ ticketIds: [ticketId], buyerSellerId }),
  });
}

describe("checkout staging-persona boundary", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("refuses a restored managed user before order or ticket access", async () => {
    ...
    expect(mockedPrisma.ticket.findMany).not.toHaveBeenCalled();
  });
