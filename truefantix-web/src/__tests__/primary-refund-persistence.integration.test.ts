...
if (!databaseUrl) describe.skip("primary refund persistence PostgreSQL integration", () => { it("requires an isolated database", () => undefined); });
  ...
  it("requires exact cancellation coverage and resolved obligations", async () => {
    ...
    const resolver=await pool.connect(); const writer=await pool.connect(); try {
      ...
      await new Promise((resolve)=>setTimeout(resolve,50)); await resolver.query("COMMIT"); await expect(blocked).rejects.toBeTruthy();
    } finally { await resolver.query("ROLLBACK").catch(()=>undefined); resolver.release(); writer.release(); }
    await expect(db.primaryCancellationBatch.create({data:{cancellationId:cancellation.id,batchKey:`late-batch-${runId}`,commandDigest:"8".repeat(64),firstTicketId:"0",lastTicketId:"z",processedTicketCount:0,processedAmountMinor:0}})).rejects.toBeTruthy();
    await expect(db.primaryCancellationSnapshotTicket.create({data:{cancellationId:cancellation.id,admissionTicketId:scope.issued[0].ticket.id,policyVersionId:"primary-refund-policy-v1",amountMinor:scope.order.grossTotalMinor,currency:"CAD"}})).rejects.toBeTruthy();
    await expect(db.primaryCancellationBatchTicket.create({data:{cancellationId:cancellation.id,batchId:batch.id,admissionTicketId:scope.issued[0].ticket.id,obligationId:obligation.id,amountMinor:scope.order.grossTotalMinor,currency:"CAD"}})).rejects.toBeTruthy();
    await expect(db.primaryObligationWaiverApproval.create({data:{obligationId:obligation.id,approvedByUserId:authorized.id,evidenceDigest:"5".repeat(64),reason:"Late waiver"}})).rejects.toBeTruthy();
    const lateRefund=await completeRefund(scope,"late-link"); await expect(db.primaryCancellationRefundLink.create({data:{cancellationId:cancellation.id,refundId:lateRefund.id}})).rejects.toBeTruthy();
    expect(await db.primaryEventCancellation.findUniqueOrThrow({where:{id:cancellation.id}})).toMatchObject({ status: "RESOLVED", processedTicketCount: 1, processedAmountMinor: scope.order.grossTotalMinor });
  });
  ...
  it("rejects cumulative obligation overuse and unrelated voluntary refund evidence", async () => {
    const scope=await seed(3); await db.$queryRaw`SELECT materialize_primary_purchase_allocations(${scope.order.id})`;
    ...
    const snapshots=await db.primaryCancellationSnapshotTicket.findMany({where:{cancellationId:cancellation.id},orderBy:{admissionTicketId:"asc"}}); expect(snapshots).toHaveLength(3);
    ...
    const batch=await db.primaryCancellationBatch.create({data:{cancellationId:cancellation.id,batchKey:`financial-batch-${runId}`,commandDigest:"7".repeat(64),firstTicketId:"0",lastTicketId:"z",processedTicketCount:3,processedAmountMinor:snapshots.reduce((sum,row)=>sum+row.amountMinor,0)}});
    ...
    const refundA=await completeRefund(scope,"linked-a",[0]); const refundB=await completeRefund(scope,"linked-b",[1]); const itemB=await db.primaryRefundItem.findFirstOrThrow({where:{refundId:refundB.id}}); const voluntarySnapshot=snapshots.find(row=>row.admissionTicketId===scope.issued[0].ticket.id)!;
    const unrelatedData={organizerId:scope.organizer.id,eventId:scope.event.id,orderId:scope.order.id,cancellationId:cancellation.id,refundId:refundA.id,cause:"EVENT_CANCELLATION",amountMinor:voluntarySnapshot.amountMinor,currency:"CAD",idempotencyKey:`unrelated-obligation-${runId}`,reason:"Retroactive voluntary refund"};
    await expect(db.primaryRefundObligation.create({data:unrelatedData})).rejects.toBeTruthy(); await expect(db.primaryCancellationRefundLink.create({data:{cancellationId:cancellation.id,refundId:refundA.id}})).rejects.toBeTruthy();

    const causalTicket=scope.issued[2].ticket.id; const causalAllocations=await db.primaryPurchaseAllocation.findMany({where:{admissionTicketId:causalTicket}}); const causalAmount=causalAllocations.reduce((sum,row)=>sum+row.amountMinor,0); const causalRefund=await db.primaryRefund.create({data:{...refundData(scope,"causal"),requestedAmountMinor:causalAmount}}); await db.primaryCancellationRefundLink.create({data:{cancellationId:cancellation.id,refundId:causalRefund.id}}); const causalItem=await db.primaryRefundItem.create({data:{refundId:causalRefund.id,admissionTicketId:causalTicket,requestedMinor:causalAmount,currency:"CAD"}}); for(const allocation of causalAllocations) await db.primaryRefundAllocation.create({data:{refundId:causalRefund.id,refundItemId:causalItem.id,admissionTicketId:causalTicket,purchaseAllocationId:allocation.id,amountMinor:allocation.amountMinor,currency:"CAD"}});
    const linked=await db.primaryRefundObligation.create({data:{...unrelatedData,refundId:causalRefund.id,amountMinor:causalAmount,idempotencyKey:`linked-obligation-${runId}`,reason:"Pre-authorized cancellation refund"}}); const itemBSnapshot=snapshots.find(row=>row.admissionTicketId===itemB.admissionTicketId)!;
    await expect(db.primaryCancellationBatchTicket.create({data:{cancellationId:cancellation.id,batchId:batch.id,admissionTicketId:itemB.admissionTicketId,refundItemId:itemB.id,obligationId:linked.id,amountMinor:itemBSnapshot.amountMinor,currency:"CAD"}})).rejects.toBeTruthy();
  });
