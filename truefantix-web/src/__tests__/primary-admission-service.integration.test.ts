...
if (!databaseUrl) describe.skip("primary admission PostgreSQL integration", () => { it("requires an isolated database", () => undefined); });
  ...
  it("atomically accepts one concurrent online scan and records later duplicates", async () => {
    ...
    const accepted = a.result === "ACCEPTED" ? { value: a, suffix: "a" } : { value: b, suffix: "b" }; const scansBefore = await db.primaryAdmissionScan.count(); const auditsBefore = await db.primaryAuditEvent.count(); const outboxBefore = await db.primaryOutboxMessage.count();
    await expect(scans.scan({ actor: { id: operator.id, role: operator.role }, organizerId: scope.organizer.id, eventId: scope.event.id, token: issued.token, requestId: `scan-concurrent-${accepted.suffix}`, deviceId: "gate-a" })).resolves.toEqual(accepted.value); expect(await db.primaryAdmissionScan.count()).toBe(scansBefore); expect(await db.primaryAuditEvent.count()).toBe(auditsBefore); expect(await db.primaryOutboxMessage.count()).toBe(outboxBefore);
    await expect(scans.scan({ actor: { id: operator.id, role: operator.role }, organizerId: scope.organizer.id, eventId: scope.event.id, token: issued.token, requestId: "scan-genuine-later", deviceId: "gate-a" })).resolves.toMatchObject({ result: "DUPLICATE" }); expect(await db.primaryAdmissionScan.count()).toBe(scansBefore + 1);
  ...
  it("returns bounded evidence codes for wrong-event, voided, unknown, tampered, and unsupported credentials", async () => {
    ...
    const unknown = signedToken({ v: 1, cid: "11111111-1111-4111-8111-111111111111", eventId: scope.event.id, iat: now.toISOString(), kid: "test_admission_1" }); const rejected = await scans.scan({ ...base, token: unknown, requestId: "scan-unknown" }); expect(rejected).toMatchObject({ result: "UNKNOWN_CREDENTIAL" }); const evidenceBefore = await db.primaryAdmissionScan.count(); const rejectedAudits = await db.primaryAuditEvent.count(); const rejectedOutbox = await db.primaryOutboxMessage.count(); await expect(scans.scan({ ...base, token: unknown, requestId: "scan-unknown" })).resolves.toEqual(rejected); expect(await db.primaryAdmissionScan.count()).toBe(evidenceBefore); expect(await db.primaryAuditEvent.count()).toBe(rejectedAudits); expect(await db.primaryOutboxMessage.count()).toBe(rejectedOutbox); await expect(scans.scan({ ...base, token: `${issued.token}x`, requestId: "scan-tampered" })).resolves.toMatchObject({ result: "INVALID_CREDENTIAL" });
  ...
  it("database constraints reject cross-ticket scan evidence and invalid telemetry", async () => {
    ...
    const base = { organizerId: scope.organizer.id, eventId: scope.event.id, operatorUserId: operator.id, commandDigest: "a".repeat(64) };
    await expect(db.primaryAdmissionScan.create({ data: { ...base, requestId: "direct-cross", admissionTicketId: issued[0].ticket.id, credentialId: issued[1].ticket.credential!.id, result: "DUPLICATE" } })).rejects.toBeTruthy();
    await expect(db.primaryAdmissionScan.create({ data: { ...base, requestId: "direct-device", result: "INVALID_CREDENTIAL", deviceId: "raw device label with spaces" } })).rejects.toBeTruthy();
    await expect(db.primaryAdmissionScan.create({ data: { ...base, requestId: "direct-unbound-accepted", result: "ACCEPTED" } })).rejects.toBeTruthy();
    await expect(db.primaryAdmissionScan.create({ data: { ...base, requestId: "direct-bound-invalid", admissionTicketId: issued[0].ticket.id, credentialId: issued[0].ticket.credential!.id, result: "INVALID_CREDENTIAL" } })).rejects.toBeTruthy();
  ...
  it("conflicts reused scan request IDs across changed token, event, operator, or device without residue", async () => {
    const scope = await seed(1); const admission = new PrimaryAdmissionService(db, capability, config, () => new Date(now)); const [issued] = await admission.issue({ internalCapability: internal, organizerId: scope.organizer.id, eventId: scope.event.id, orderId: scope.order.id, idempotencyKey: "scan-conflict-issue" }); const firstOperator = await scanner(scope); const secondOperator = await scanner(scope); const other = await seed(1); const otherOperator = await scanner(other); const scans = new PrimaryAdmissionScanService(db, capability, admission); const requestId = "scan-conflict-request"; const original = { actor: { id: firstOperator.id, role: firstOperator.role }, organizerId: scope.organizer.id, eventId: scope.event.id, token: issued.token, requestId, deviceId: "gate-a" };
    await expect(scans.scan(original)).resolves.toMatchObject({ result: "ACCEPTED" }); const scansBefore = await db.primaryAdmissionScan.count(); const auditsBefore = await db.primaryAuditEvent.count(); const outboxBefore = await db.primaryOutboxMessage.count();
    for (const changed of [{ ...original, token: `${issued.token}x` }, { ...original, deviceId: "gate-b" }, { ...original, actor: { id: secondOperator.id, role: secondOperator.role } }]) await expect(scans.scan(changed)).rejects.toMatchObject({ code: "SCAN_IDEMPOTENCY_CONFLICT" });
    await expect(scans.scan({ ...original, actor: { id: otherOperator.id, role: otherOperator.role }, organizerId: other.organizer.id, eventId: other.event.id })).rejects.toMatchObject({ code: "SCAN_IDEMPOTENCY_CONFLICT" });
    expect(await db.primaryAdmissionScan.count()).toBe(scansBefore); expect(await db.primaryAuditEvent.count()).toBe(auditsBefore); expect(await db.primaryOutboxMessage.count()).toBe(outboxBefore);
  });
});
