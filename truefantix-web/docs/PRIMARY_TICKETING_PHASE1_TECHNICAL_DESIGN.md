...
- `REQUESTED -> PROVIDER_PENDING` calls a database completeness assertion under the parent lock. Item totals must equal the parent request, each item's allocation total must equal the item, and the aggregate allocation total must equal the request. Each purchase allocation is locked while cumulative use is checked, so concurrent refunds cannot exceed it. A provider attempt must exactly equal the reconciled parent amount/currency.
- A `CHECKED_IN` ticket cannot become a refund item unless a prior immutable `PrimaryCheckedInRefundApproval` binds that exact refund/ticket, a current verified non-banned named approver, evidence digest, reason, fraud review, and explicit `ORGANIZER | TRUEFANTIX` cost bearer. Unscanned tickets do not require this exception record.
- Cancellation counters are derived only when entering `RESOLVED`; caller edits are rejected. Immutable `PrimaryCancellationBatchTicket` rows bind a non-overlapping ticket claim to its cancellation, batch range, exact-event ticket, order-scoped obligation, optional exact-ticket refund item, amount, and currency. Every batch's declared totals and the generation's complete totals must reconcile. A partial unique index permits at most one active/refunding/reconciliation generation per event.
- `WAIVED_WITH_APPROVAL` requires a prior immutable named approval with evidence digest and reason. Independent foreign keys are supplemented with insertion guards requiring obligation, refund, cancellation, revocation, order, event, organizer, policy, ticket/credential, amount, and currency scope to agree.
