...
- The authoritative sequence is: serializable local `HELD -> PAYMENT_COMMITTED` and `PENDING_PAYMENT -> PAYMENT_PROCESSING` commit; durable scoped attempt creation; only then an adapter call creating an unconfirmed test PaymentIntent. The service returns the persisted attempt, never a client secret, and there is no checkout/public route.
...
- Attempt states are `PENDING_PROVIDER`, `PROCESSING`, `SUCCEEDED`, `FAILED`, `CANCELLED`, and `RECONCILIATION_REQUIRED`. Signed provider events are stored only as safe identifiers/type/time plus a SHA-256 body digest. Duplicate events are no-ops; terminal success dominates later failures.
