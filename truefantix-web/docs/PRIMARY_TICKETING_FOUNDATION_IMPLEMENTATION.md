...
- Command-replay hardening rejects a repeated checked-in refund request before it can emit duplicate request audit evidence. The PostgreSQL console suite proves the second command fails with a stable rejection code and the successful request leaves exactly one audit record. Verification on a fresh disposable PostgreSQL 16 database deployed all 53 migrations and passed 68 suites / 453 tests with integration enabled; Prisma format/validation/generation/status, TypeScript, full lint with no errors, production build, and diff checks passed.
