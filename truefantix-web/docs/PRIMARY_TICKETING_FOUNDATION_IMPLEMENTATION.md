...
- `PrimaryAdmissionCredential` is one immutable generation in this milestone. Its ID is a cryptographically random UUID. The canonical Ed25519 payload is `{"v":1,"cid":"<credential UUID>","eventId":"<primary event ID>","iat":"<ISO-8601 UTC>","kid":"<test key ID>"}`. It contains no buyer identity, contact data, price, seat, or order data. The database stores the signature and SHA-256 payload digest, never a private key or complete bearer token.
