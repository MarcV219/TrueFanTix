...
- The authoritative sequence is: reserve the scoped attempt command/idempotency key without changing inventory; reject any cross-order/scope key reuse; complete the serializable local `HELD -> PAYMENT_COMMITTED` and `PENDING_PAYMENT -> PAYMENT_PROCESSING` commit; only then call the adapter to create an unconfirmed test PaymentIntent. The service returns the persisted attempt, never a client secret, and there is no checkout/public route.
