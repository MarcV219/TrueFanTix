-- Preserve every deployed transition migration and freeze retryable failure
-- evidence until the row either acquires its next due claim or is escalated for
-- reconciliation. A same-state update must not rewrite the rejection evidence
-- or move its durable retry schedule.
...
BEGIN
  ...
  IF OLD.status = 'FAILED'
    ...
    AND ROW(
      NEW.provider, NEW."attemptCount", NEW."firstAttemptAt",
      NEW."processingAt", NEW."leaseExpiresAt", NEW."claimToken",
      NEW."dispatchStartedAt", NEW."deliveredAt", NEW."lastError",
      NEW."availableAt"
    ) IS DISTINCT FROM ROW(
      OLD.provider, OLD."attemptCount", OLD."firstAttemptAt",
      OLD."processingAt", OLD."leaseExpiresAt", OLD."claimToken",
      OLD."dispatchStartedAt", OLD."deliveredAt", OLD."lastError",
      OLD."availableAt"
    ) THEN
    RAISE EXCEPTION 'Failed transfer-proof delivery evidence is immutable until retry or reconciliation';
  END IF;
  ...
  IF OLD.status IS DISTINCT FROM 'FAILED'
    ...
    RAISE EXCEPTION 'Transfer-proof delivery failure requires its owned dispatch boundary';
  END IF;
