-- Preserve every deployed transition migration and require retryable failure
-- evidence to descend from the currently recorded provider dispatch boundary.
-- A claimed retry that has not crossed the provider boundary cannot be marked
-- FAILED, and its next-attempt schedule cannot predate that dispatch.
