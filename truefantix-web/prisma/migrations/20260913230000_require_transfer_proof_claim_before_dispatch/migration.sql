-- Preserve every deployed transition migration and require a due claim to be
-- acquired before its provider boundary is recorded. In particular, a FAILED
-- retry cannot combine claim acquisition with a dispatch marker while leaving
-- the attempt count unchanged, and neither a new nor retry claim can bypass
-- its durable available-at schedule.
