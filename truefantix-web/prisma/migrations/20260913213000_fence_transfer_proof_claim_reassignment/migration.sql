-- Preserve all deployed transition migrations and tighten active claim
-- reassignment with a forward-only function replacement. A dispatched claim
-- may be reset only by a new owner after the prior logical lease expires.
...
BEGIN
  ...
  IF OLD.status = 'PROCESSING'
    ...
    RAISE EXCEPTION 'Transfer-proof delivery active claim evidence requires an expired-lease handoff';
  END IF;
