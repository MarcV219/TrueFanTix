-- Preserve every deployed delivery transition migration and tighten only
-- future expired-lease handoffs. A claim that crossed the provider boundary
-- may be replayed only through Resend's still-active 24-hour idempotency
-- window; SendGrid dispatches and expired Resend identities require
-- reconciliation instead of a new claim.
